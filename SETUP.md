...
console.log("Environment check:", {
  ...
  credentialsFile: require("fs").existsSync("./credentials.json")
    ? "Found"
    : "Missing",
...
- Node.js support enabled via Application Manager
- SSH access to your account
- mod_passenger installed (for running Node.js apps)

### Deployment Steps

#### 1. Prepare Your Environment

**On your local machine**, update `.env` for production:
...
# Add your credentials (keep these secure!)
...
tar -czf notion-sync.tar.gz --exclude=node_modules --exclude=.git .

# Upload to server
scp notion-sync.tar.gz username@yourserver.com:~/

# SSH into your server
ssh username@yourserver.com

# Extract files
mkdir ~/notion-google-task-sync
cd ~/notion-google-task-sync
tar -xzf ../notion-sync.tar.gz
```
...
3. Extract the archive in your home directory
...
# Install dependencies using the system Node.js
npm install --production
```

#### 4. Complete OAuth Setup

You need to run OAuth **once** to generate `token.json`:

```bash
# Run a one-time sync to trigger OAuth
node app.js

# Follow the authentication prompts
# This creates token.json which is used for subsequent syncs
```

**Note**: If you can't run a browser on the server, complete OAuth locally first, then upload `token.json` to the server.

#### 5. Register Application in cPanel Application Manager

1. Log in to cPanel
2. Navigate to **Software** → **Application Manager**
3. Click **Register Application**
4. Fill in the configuration:
   - **Application Name**: `notion-google-sync` (no periods)
   - **Deployment Domain**: Your domain (e.g., `yourdomain.com`)
   - **Base Application URL**: Leave empty or use `/sync` if you want it at a subdomain
   - **Application Path**: `notion-google-task-sync` (relative to home directory)
   - **Deployment Environment**: Select **Production**
5. Add environment variables using the interface (all variables from `.env`)
6. Click **Deploy**

#### 6. Configure the Application Entry Point

Edit the `.htaccess` file in your application directory:
...
Update the `PassengerAppRoot` line with your actual username:

```apache
PassengerAppRoot /home/YOUR_USERNAME/notion-google-task-sync
```

#### 7. Start the Scheduled Sync
...
2. Add a new cron job:
   ...
   - **Command**:
     ```bash
     cd /home/YOUR_USERNAME/notion-google-task-sync && /usr/bin/node app.js
     ```
...
```bash
# If using cron
cat ~/notion-google-task-sync/sync.log

# If using built-in scheduler
tail -f ~/notion-google-task-sync/sync.log
```
...
tail -n 50 ~/notion-google-task-sync/sync.log
```
...
#### "Application not found" in cPanel

- Verify the application path matches your directory structure
- Check that `app.js` exists in the specified directory
...
- Check cron job email for error messages
- Verify the full path to node: `which node`
- Test the command manually first
- Ensure `.env` file is present and readable

#### Performance Issues

- Increase `SYNC_INTERVAL_MINUTES` to reduce API load
- Check Notion/Google API rate limits
...
1. **Protect sensitive files**:
   ...
   ```bash
   chmod 600 .env token.json credentials.json
   ```
...
3. **Use environment variables** in cPanel Application Manager instead of `.env` file when possible

4. **Regular backups**:
   ```bash
   # Backup token and credentials
   tar -czf notion-sync-backup.tar.gz .env token.json credentials.json
   ```
...
cp -r ~/notion-google-task-sync ~/notion-google-task-sync.backup

# Upload new files (via git pull or file upload)
